...
This is a model that simulates how mating preferences and fertility differentials by BMI influence the distribution of Body Mass Index (BMI) across generations. It explores how assumptions about who partners with whom (assortative mating) and how many children they have (differential fertility) can change the population's health demographics over time.

## HOW IT WORKS

Agents: Each "turtle" represents a person with attributes like genetic predisposition to BMI (PGS-BMI), actual BMI, BMI category, and gender.

Mating rules (omega): A slider controls the proportion of the population that practices assortative mating (preferring partners in the same BMI group). Those who can't find a same-group partner, and the rest of the population, mate randomly.

Fertility differentials (phi): A slider controls the strength of differential fertility, where a couple's BMI group influences their number of offspring. A value of 0 means all couples have the same number of children (2).

Inheritance: Children inherit a genetic score (PGS-BMI) from their parents, which is used to calculate their own BMI. This connects one generation to the next.


## HOW TO USE IT

SETUP: Click the setup button to create the initial population.

GO: Click the go button to run the simulation continuously. Click it again to pause.

Sliders:

initial-people: The number of individuals in the starting generation.

omega (ω): Controls mating. At 1, everyone tries to mate with their same BMI group. At 0, all mating is random.

phi (φ): Controls fertility differences. At 0, all couples have 2 children. Higher values increase the effect of BMI on family size.

## THINGS TO TRY

Additionally, the slider "p" produces scenarios with increased genetic penetrance. This can be used to simulate the effect of policies that increase the penetrance via gene-environment interactive effects.

## CREDITS AND REFERENCES

Aitor García Aguirre. 
Affiliations: Spanish National Research Council; University Carlos III of Madrid & ECHO-ERC Project.
@#$#@#$#@
default
true
0
